        program ray
c
c --- einfaches strahlverfolgungsprogramm im 3-strahlfall 
c     interferometer 
...
c strahlen
        ...
        real Intensitaet(50),Sum_Intensitaet(50)
        real Int_Detail(10),Sum_Detail(10),Sum
...
c        open(13,'strahl.dat')
         open(30,file='rock_temp.dat')
         open(31,file='rockd_temp.dat')        
         

c
111     format(22f12.6,i4)
112     format(13f12.6)
c
        pi = 4.0*atan(1.0)
        dtp = pi/180.0
...
c        write(6,*) ' Winkel '
c        read(5,*) winkel
c        bragg = 15.379296 !Si 111 bei 1.66289 Angstr.
        bragg = 15.37731167 !Si 111 bei 1.66289 Angstr. mit Brechungskorrektur
        t = -bragg*dtp
        call matrix(0.0,1.0,0.0,t,m)
c
...
c
        call histinit(1,1.66239,1.66339,20)
...
c       
            ...
            lambda = 1.66289 + (lambda_x-0.5)*100.e-5
...
c 1. monochromatorspiegel
     ...
     .            s1_v,s1_u,s1_n,neu1_p0,neu1_a,winkel)
            ...
            if(hit_it.eq.yes) then 
              winkel = winkel*ptd
              intensitaet(l_ix) = reflex(lambda,winkel)
              if(intensitaet(l_ix). eq. 0. ) goto 100
            else 
              goto 100
            endif
...
c          
            if(hit_it.eq.yes) then 
               winkel = winkel*ptd 
               intensitaet(l_ix) = intensitaet(l_ix)
...
c blende 2
     ...
     .            b2_v,b2_u,b2_n,neu3_p0,neu3_a,winkel)
            if(hit_it.eq.yes) then 
              Sum_Intensitaet(l_ix) = Sum_Intensitaet(l_ix) + 
     .                                Intensitaet(l_ix)
              number_hits = number_hits + 1
              else 
              goto 100
            endif
c         call histogram(1,lambda)  
c        Write(6,*) ' Sum: ',Sum_Intensitaet
         if(l_ix_det .ge. 0) then 
              sum_detail(l_ix_det) = Sum_intensitaet(l_ix)
         endif
         sum = 0.
          do j=1,20
             sum = sum + Sum_intensitaet(l_ix)
          end do
100      continue
        
         end do
         write(30,111) dp,bragg+dpp,(Sum_intensitaet(j),j=1,20),
     .                 number_hits
         write(31,112) dp,bragg+dpp,(sum_detail(j),j=1,10),sum
c         write(6,*) dp
        end do
c        close(13)
        close(30)
c        call histowrite(1,'l_ver.dat')
        end
        stop 